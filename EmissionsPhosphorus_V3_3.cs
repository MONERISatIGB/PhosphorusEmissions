...
namespace Moneris.Core.Engine
{
    ...
    public class EmissionsPhosphorus : BaseEngineClass, Moneris.Core.Engine.IEmissionsPhosphorus
    {
        ...
        public override void Calc(Result result)
        {
            ...
            {
                if (!option.PFreeDishwaterDetergents)
                    pfree_ratio = (country.PhosphorInhabitant - country.PhosphorLaundryDetergent - country.PhosphorDishwaterDetergent) / country.PhosphorInhabitant;
                else
                    pfree_ratio = (country.PhosphorInhabitant - country.PhosphorDishwaterDetergent) / country.PhosphorInhabitant;
            }
            else if (!option.PFreeDishwaterDetergents)
            {
                if (!option.PfreeLaundryDetergents)
                    ...
                    pfree_ratio = (country.PhosphorInhabitant - country.PhosphorLaundryDetergent) / country.PhosphorInhabitant;
            }
            else
                pfree_ratio = 1;
            #region Comments:
            //Changed 17112015 by Markus
            //this equation is still unchanged from Behrendt original Version. It is unclear why the inhabitant specific P release has to be corrected and why "/0.850515" is added. Origing of this 
            //factor is unknown.
            ...
            //For standard calculation the pfree_ratio is set to 1. For scenarios "/0.850515" is removed.
            #endregion
            ...
            ////The approach is based on results by Pöthig et al. (2010), Here, water soluble P (WSP) was analysed for several hunderet soils samples
            ////in Germany and Swiss. From the WSP the degree of saturation  (DPS) was derived, a soil type independent approach to derive the risk of P
            ////dissolved P losses. In MONERIS we estimate DPS from P-accumulation on agricultural soils and the soil density for different soil types.

            //double p_sat_max = 97;
            //#region Comments:
            ////Units [%]
            ////P_sat_max defines the maixukum allowable P saturation, as the approach by Pöthig has not beeen 
            ////tested and validated for higher values
            //#endregion
            ...
            //double VolumeWeightSandySoils = 1.1 * 1000000 * 0.30 / 1000  * 10000;
            //#region Comments:
            ////Units:    [kg/ha]         = [g/cm³]*[cm³/m³] * [m] / [g/kg]* [m²/ha]    
            ////Volume weight of sandy soils
            ////spcefific weight of sandy soils = 1.1 g/cm³
            ////considered top soil depth = 30 cm
            //#endregion

            //double VolumeWeightLoamySiltySoils = 1.5 * 1000000 * 0.30 / 1000 * 10000;
            //#region Comments:
            ////Units:    [kg/ha]         = [g/cm³]*[cm³/m³] * [m] / [g/kg]* [m²/ha]    
            ////Volume weight of silty soils
            ////spefific weight of loamy-silty soils = 1.5 g/cm³
            ////top soil depth = 30 cm
            //#endregion

            //double VolumeWeightOrganicSoils = 0.5 * 1000000 * 0.30 / 1000 * 10000;
            //#region Comments:
            ////Units:    [kg/ha]         = [g/cm³]*[cm³/m³] * [m] / [g/kg]* [m²/ha]    
            ////Volume weight of sandy soils
            ...
            ////P content in upper 30 cm of soils
            ...
            ////WSP derived from P content according to Pöthig et al. (2010)
            //double WSPSandySoils = 0.0451 * PContentSandySoils;  // in mg/kg
            //double WSPLoamySiltySoils = 0.018 * PContentLoamySiltySoils;  // in mg/kg
            //double WSPOrganicSoils = 0.0043 * PContentOrganicSoils;  // in mg/kg

            
            
            ////Hier muss noch die korrektur der WSP-Pöthig in WSP-Vadas eingebaut werden!!


            ////P concentration in surface run-off according to Vadas et al. (2005)
            //double P_SR_SandySoils = (11.2 * WSPSandySoils - 66.9) / 1000;  // in mg/l
            //double P_SR_LoamySiltySoils = (11.2 * WSPLoamySiltySoils - 66.9) / 1000;  // in mg/l
            //double P_SR_OrganicSoils = (11.2 * WSPOrganicSoils - 66.9) / 1000;  // in mg/l
            //double P_SR_AgriculturalLand = 0.0;

            ////Calculation of area weighted mean concentration
            //if (soil.SandySoil + soil.SiltySoil + soil.LoamySoil + landUse.BogDegraded + landUse.FenDegraded != 0.0)
            //{
            //    P_SR_AgriculturalLand = (P_SR_SandySoils * soil.SandySoil + P_SR_LoamySiltySoils * (soil.SiltySoil + soil.LoamySoil) + P_SR_OrganicSoils * (landUse.BogDegraded + landUse.FenDegraded)) / (soil.SandySoil + soil.SiltySoil + soil.LoamySoil + landUse.BogDegraded + landUse.FenDegraded);
            //}
            
            //#region Estimate of degree of saturation
            ////according to Pöthig et al 2010 the degree of saturation can be derived from WSP
            //double PSaturationSandy = 1 / (1 + 1.25 * Math.Pow(WSPSandySoils, -0.75));
            //double PSaturationLoamySity = 1 / (1 + 1.25 * Math.Pow(WSPLoamySiltySoils, -0.75));
            //double PSaturationOrganic = 1 / (1 + 1.25 * Math.Pow(WSPOrganicSoils, -0.75));

            //double PSaturationAgriculturalLand = 50.0;

            ////Calculation of area weighted mean of saturation
            //if (soil.SandySoil + soil.SiltySoil + soil.LoamySoil + landUse.BogDegraded + landUse.FenDegraded != 0.0)
            //{
            //    PSaturationAgriculturalLand = (PSaturationSandy * soil.SandySoil + PSaturationLoamySity * (soil.SiltySoil + soil.LoamySoil) + PSaturationOrganic * (landUse.BogDegraded + landUse.FenDegraded)) / (soil.SandySoil + soil.SiltySoil + soil.LoamySoil + landUse.BogDegraded + landUse.FenDegraded);
            //}

            ////Checking for too high Saturation values
            //if (PSaturationAgriculturalLand > p_sat_max)
            //    PSaturationAgriculturalLand = p_sat_max;
            //#endregion
            //#endregion
            
            ////TP-Konzentration im Oberflächenabfluss von landwirtschaftlichen Nutzflächen(P_SR_conc_AL)
            //double pConcentrationArableLand = P_SR_AgriculturalLand;

            ////TP-Konzentration im Oberflächenabfluss von Grünland(P_SR_conc_GL)
            //double pConcentrationGrassland = P_SR_AgriculturalLand;
            ...
            #region new version using DPS from DB separately for arable land and grassland
            //degree of phosphorus saturation (DPS) has been calculated in pre-processing in basis of Pöthig et al. as described in the former approach
            //DPS has been derived for arable land and grassland separately and been safed as Test1 and Test2 in analytical data table
            double WSP_max = 70;
            #region Comments:
            //Units [%]
            //P_sat_max defines the maixukum allowable P saturation, as the approach by Pöthig has not beeen 
            ...
            double WSP_arable = basics.Test1;
            double WSP_grass = basics.Test2;
            ...
            if (WSP_grass > WSP_max)
                WSP_grass = WSP_max;

            if (WSP_arable > WSP_max)
                WSP_arable = WSP_max;

            //In all former Versions the soil P equilibrium concentration was used for tsurface run-off. 
            //Vadas et al. 2005 compared surface run-off concentrations to WSP.
            //This correlation is used in the new appraoch

            double pConcentrationArableLand = 0.01;
            double pConcentrationGrassland = 0.01;

            double WSP_correctionFactor = 0.7936;
            #region comments:
            //Correction factor according to Fischer et al 2016
            //This Factor consideres the slightly increased concentrations derived by Vadas et al. (2005) 
            //which orriginate from different used soil to water ratios
            #endregion

            pConcentrationArableLand = ((11.2 * WSP_arable + 66.9) / 1000) * WSP_correctionFactor;
            pConcentrationGrassland = ((11.2 * WSP_grass + 66.9) / 1000) * WSP_correctionFactor;
            #region Comments:
            //gives the filterable reactive according to Vadas et al. (2005)
            //Units: mg/l
            #endregion

            //P concentration in SR was considered by constants. In general P conc in surface run-off is very low, but it can vary accoriding to the dominating soil types.
            //DPS for unfertilized soils are assumed to be:
            //sandy: 20 %
            //silty/loamy/clay: 10 %
            double WSPUnfertilizedAreas = 1.0;

            if (soil.SandySoil + soil.SiltySoil + soil.LoamySoil + soil.ClayeySoil != 0.0)
            {
                WSPUnfertilizedAreas = (1.5 * soil.SandySoil + 1.0 * (soil.SiltySoil + soil.LoamySoil + soil.ClayeySoil))/(soil.SandySoil + soil.SiltySoil + soil.LoamySoil + soil.ClayeySoil);
            }

            double PconcSrUnfertilizedAreas = 0.01;
            //0.1615 is the P concentration in surface run-off at DPS = 15 % according to Vadas
            PconcSrUnfertilizedAreas = (11.2 * WSPUnfertilizedAreas + 66.9) / 1000 * WSP_correctionFactor;
            ...
            result.TPSurfaceRunoffNaturalCovered = basics.QNaturalCovered * PconcSrUnfertilizedAreas * 86.4 * 0.365; //(P_SR_NatCov) 

            result.TPSurfaceRunoffOpenArea = basics.QOpenArea * PconcSrUnfertilizedAreas * 86.4 * 0.365; //(P_SR_OA) 

            double pOpenPitMine = basics.QOpenPitMine * PconcSrUnfertilizedAreas * 86.4 * 0.365; //(P_SR_OPM) 

            double pWetland = basics.QWetland * PconcSrUnfertilizedAreas * 86.4 * 0.365; //(P_SR_Wetland) 

            result.TPSurfaceRunoffSnow = result.Q_Snow * Constants.CSR12 * 86.4 * 0.365; //(P_SR_Snow) 
            //For snow/ice covered areas are concentration constants are still used

            //Check concentrations for Wetland, fen and bogs!!
            ...
            //Changed 04052014 by Markus changes of P accumulation over time was not consdered by former version.
            //double pContentTopSoil = soil.CorrectPhosphorContent(country.PhosphorAccum);//CD_Pcont, ER_TS_TPcont

            double pContentTopSoil = soil.CorrectPhosphorContent(p_acc_corr) ;//CD_Pcont, ER_TS_TPcont
            ...
            #region Comments:
            //Units:       [t/yr]      =     [mg/kg]    /   [t/mg]   *       [t/yr]                   *         [-]  
            //changed by Markus 0308215
            //For some model applications it was test to set SDR = 1 if slope is higher than 10 %  --> erosion from entire forest would reach surface waters
            //has been remved for current version
            //if (analyticalUnit.Slope1000 > 10.0)
            //    result.TPErosionNaturalCovered = basics.SoilLossNaturalCovered * Constants.CE13 * basics.EnrichmentRatio / 1000000.0; //(ER_TP_NatCov) 
            #endregion
        ...
        {
            get { return "corrected reading of TP accumulation from Database, Phosphate from inhabitants corrected"; }
        }
    }
}
